...
Real FaceToCenterInterp(const ReconstructionMethod recon,
  ...
  switch (recon) {
    ...
    case ReconstructionMethod::teno:
      TENO(bf_im2, bf_im1, bf_i, bf_ip1, bf_ip2, qL, dum);   // center face i → qL at cc
      TENO(bf_im1, bf_i, bf_ip1, bf_ip2, bf_ip3, dum, qR);   // center face i+1 → qR at cc
      return 0.5*(qL + qR);
    case ReconstructionMethod::ppm:
      // Unlimited (MOOD-protected) base: cell-centered B uses the 2nd-order face average.
      // A genuinely 4th-order face-to-center for this path, e.g.
      //   (7*(bf_i + bf_ip1) - (bf_im1 + bf_ip2))/12, is a deferred accuracy improvement.
      return 0.5*(bf_i + bf_ip1);
  }
...
void IdealMHD::ConsToPrim(DvceArray5D<Real> &cons, const DvceFaceFld4D<Real> &b,
  ...
  const bool ho_bcc = (pmy_pack->pmhd->emf_method != MHD_EMF::ct_contact);
  ...
  KOKKOS_LAMBDA(const int &idx, int &sumd, int &sume, int &sumt) {
    ...
    if (only_testfloors) {
      ...
      // standard 2nd-order face-to-center average (ct_contact EMF path)
      ...
      // method as the flux computation (UCT EMF path).  Treats face-averaged B
      // values as "cell averages" in a dual grid, consistent with the FV framework.
