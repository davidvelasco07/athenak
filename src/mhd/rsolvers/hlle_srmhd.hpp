...
//========================================================================================
//! \file hlle_srmhd.hpp
//! \brief HLLE Riemann solver for special relativistic MHD.  Per-face implementation for
...
//----------------------------------------------------------------------------------------
//! \fn HLLE_SR<ivx>()
//! \brief The HLLE Riemann solver for SR MHD, single face (m,k,j,i).
template <int ivx>
...
void HLLE_SR(const EOS_Data &eos,
  ...
  Real ux_l = wl(m,ivx,k,j,i);
  Real uy_l = wl(m,ivy,k,j,i);
  Real uz_l = wl(m,ivz,k,j,i);
  Real u_l[4];
  ...
  Real ux_r = wr(m,ivx,k,j,i);
  Real uy_r = wr(m,ivy,k,j,i);
  Real uz_r = wr(m,ivz,k,j,i);
  Real u_r[4];
  ...
  Real pgas_l, pgas_r;
  pgas_l = eos.IdealGasPressure(wl(m,IEN,k,j,i));
  pgas_r = eos.IdealGasPressure(wr(m,IEN,k,j,i));
  ...
  if (compute_uct) {
    ...
    uct_dL(m,k,j,i)  = dval;
    uct_dR(m,k,j,i)  = dval;
    uct_vt1(m,k,j,i) = (alpha_r*uy_l + alpha_l*uy_r)*isum;
    uct_vt2(m,k,j,i) = (alpha_r*uz_l + alpha_l*uz_r)*isum;
  }
}
} // namespace mhd
