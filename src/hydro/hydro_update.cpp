...
TaskStatus Hydro::RKUpdate(Driver *pdriver, int stage) {
  ...
  // Match fallback-validation: prescribed kinematic flow advects only scalars.
  // Evolving the background would change the velocity field being prescribed.
  int n_lo = 0;
  if (pdriver->time_evolution == TimeEvolution::kinematic) {
    if (nscalars <= 0) return TaskStatus::complete;
    n_lo = nhydro;
  }
  auto u0_ = u0;
  auto u1_ = u1;
  ...
  par_for_outer("h_update",DevExeSpace(),scr_size,scr_level,0,nmb1,n_lo,nvar-1,ks,ke,js,je,
