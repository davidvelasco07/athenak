...
void Hydro::MOODLoop(Driver *pdriver, int stage) {
  ...
  for (int rev=1; rev<=nrevs; ++rev) {
    ...
    if (scale_mode == 2 && rev == 1) {
      ...
      KOKKOS_LAMBDA(const int &idx, Real &dd, Real &de) {
        int m = idx/nkji_a;
      ...
      }, Kokkos::Max<Real>(gdd), Kokkos::Max<Real>(gde));
#if MPI_PARALLEL_ENABLED
      Real rmx[2] = {gdd, gde};
      MPI_Allreduce(MPI_IN_PLACE, rmx, 2, MPI_ATHENA_REAL, MPI_MAX, MPI_COMM_WORLD);
      gdd = rmx[0]; gde = rmx[1];
#endif
      gscale0 = gdd;
      gscale1 = gde;
