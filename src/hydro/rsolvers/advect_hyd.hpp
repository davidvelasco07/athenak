...
void Advect(TeamMember_t const &member, const EOS_Data &eos,
  ...
  par_for_inner(member, il, iu, [&](const int i) {
    ...
    if (wl(ivx,i) >= 0.0) {
      ...
      flx(m,ivy,k,j,i) = wl(IDN,i)*wl(ivy,i)*wl(ivx,i);
      flx(m,ivz,k,j,i) = wl(IDN,i)*wl(ivz,i)*wl(ivx,i);
      if (eos.is_ideal) {
      ...
      flx(m,ivy,k,j,i) = wr(IDN,i)*wr(ivy,i)*wr(ivx,i);
      flx(m,ivz,k,j,i) = wr(IDN,i)*wr(ivz,i)*wr(ivx,i);
      if (eos.is_ideal) {
