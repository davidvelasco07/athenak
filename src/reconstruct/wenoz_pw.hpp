#ifndef RECONSTRUCT_WENOZPW_HPP_
#define RECONSTRUCT_WENOZPW_HPP_
//========================================================================================
...
void WENOZPW(const Real &q_im2, const Real &q_im1, const Real &q_i, const Real &q_ip1,
  ...
  const Real d_coeff[3]{1./16, 5./8., 5./16};

  Real beta[3];
  beta[0] = beta_coeff[0] * SQR(q_im2 - 2.0*q_im1 +     q_i) +
            beta_coeff[1] * SQR(q_im2 + 3.0*q_i - 4.0*q_im1);

  beta[1] = beta_coeff[0] * SQR(q_im1 - 2.0*q_i + q_ip1) +
            beta_coeff[1] * SQR(q_im1 -     q_ip1);

  beta[2] = beta_coeff[0] * SQR(q_ip2 - 2.0*q_ip1 +      q_i) +
            beta_coeff[1] * SQR(q_ip2 + 3.0* q_i - 4.0*q_ip1);

  // Rescale epsilon
  const Real epsL = 1.0e-40;
  ...
  // compute qL_ip1
  // Factor of 1/8 in coefficients of f[] array applied to alpha_sum to reduce divisions
  Real f[3];
  f[0] = ( 3.0*q_im2 - 10.0*q_im1 + 15.0*q_i   );
  f[1] = ( 3.0*q_ip1 + 6.0*q_i    -      q_im1 );
  f[2] = ( 3.0*q_i   + 6.0*q_ip1  -      q_ip2 );

  Real alpha[3];
  alpha[0] = d_coeff[0]*(1.0 + indicator[0]);
  alpha[1] = d_coeff[1]*(1.0 + indicator[1]);
  alpha[2] = d_coeff[2]*(1.0 + indicator[2]);
  Real alpha_sum = 8.0*(alpha[0] + alpha[1] + alpha[2]);

  ql_ip1 = (f[0]*alpha[0] + f[1]*alpha[1] + f[2]*alpha[2])/alpha_sum;

  // compute qR_i
  // Factor of 1/8 in coefficients of f[] array applied to alpha_sum to reduce divisions
  f[0] = ( 3.0*q_ip2 - 10.0*q_ip1 + 15.0*q_i   );
  f[1] = ( 3.0*q_im1 + 6.0*q_i    -      q_ip1 );
  f[2] = ( 3.0*q_i   + 6.0*q_im1  -      q_im2 );

  alpha[0] = d_coeff[0]*(1.0 + indicator[2]);
  alpha[1] = d_coeff[1]*(1.0 + indicator[1]);
  alpha[2] = d_coeff[2]*(1.0 + indicator[0]);
  alpha_sum = 8.0*(alpha[0] + alpha[1] + alpha[2]);
...
KOKKOS_INLINE_FUNCTION
void WENOZPWX1(TeamMember_t const &member, const EOS_Data &eos, const bool apply_floors,
  ...
  for (int n=0; n<nvar; ++n) {
    par_for_inner(member, il, iu, [&](const int i) {
      ...
      WENOZPW(qim2, qim1, qi, qip1, qip2, ql(n,i+1), qr(n,i));
      if (apply_floors) {
        if (n==IDN) {
...
KOKKOS_INLINE_FUNCTION
void WENOZPWX2(TeamMember_t const &member, const EOS_Data &eos, const bool apply_floors,
     const int m, const int k, const int j, const int il, const int iu,
  ...
  for (int n=0; n<nvar; ++n) {
    par_for_inner(member, il, iu, [&](const int i) {
      ...
      WENOZPW(qjm2, qjm1, qj, qjp1, qjp2, ql_jp1(n,i), qr_j(n,i));
      if (apply_floors) {
        if (n==IDN) {
...
KOKKOS_INLINE_FUNCTION
void WENOZPWX3(TeamMember_t const &member, const EOS_Data &eos, const bool apply_floors,
     const int m, const int k, const int j, const int il, const int iu,
  ...
  for (int n=0; n<nvar; ++n) {
    par_for_inner(member, il, iu, [&](const int i) {
      ...
      WENOZPW(qkm2, qkm1, qk, qkp1, qkp2, ql_kp1(n,i), qr_k(n,i));
      if (apply_floors) {
        if (n==IDN) {
...
#endif // RECONSTRUCT_WENOZPW_HPP_
